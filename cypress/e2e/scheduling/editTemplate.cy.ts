    ...
    cy.get(CyUtils.getSelector(CypressIds.PAGE_SCHEDULING_TEMPLATES_TEMPLATES_TABLE))
      ...
      .then((table) => {
        ...
        cy.wait(2000).then(() => {
          ...
          if (tableLength > 0) {
            cy.get(CyUtils.getSelector(CypressIds.PAGE_SCHEDULING_TEMPLATES_CHECKBOX_SELECT_ALL))
              .should('exist')
              .realClick();
            // eslint-disable-next-line cypress/no-unnecessary-waiting
            cy.wait(2000);
            cy.get(CyUtils.getSelector(CypressIds.PAGE_SCHEDULING_TEMPLATES_BUTTON_DELETE)).should('exist').click();
            cy.get(CyUtils.getSelector(CypressIds.MODAL_SCHEDULING_DELETION_BUTTON_CONFIRM))
              .should('exist')
              .realClick();
            cy.get(CyUtils.getSelector(CypressIds.PAGE_SCHEDULING_TEMPLATES_SUCCESSFULLY_DELETED_MESSAGE)).should(
              'exist'
            );
