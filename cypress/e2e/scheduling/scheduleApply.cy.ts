...
describe('apply schedule', () => {
  ...
  it(`should apply schedule with service type template and it displays in calendar`, () => {
    cy.fixture('test-data').then((data) => {
      ...
      cy.get(CyUtils.getSelector(CypressIds.PAGE_SCHEDULING_CREATE_TEMPLATES_PLACEHOLDER)).type(
        data.apply_schedule_comment_1
      );
      ...
      cy.ChooseProvider(data.service_provider_2).then(() => {
        cy.SelectDate(nextSaturdayDate).then(() => {
  ...
  it(`should apply schedule with block template and it displays in calendar`, () => {
    cy.fixture('test-data').then((data) => {
      ...
      cy.url().should('include', '/booking/appointments');
      cy.ChooseProvider(data.service_provider_2).then(() => {
        cy.SelectDate(nextSundayDate).then(() => {
          cy.get(CyUtils.getSelector(CypressIds.COMMON_FULL_CALENDAR_LOADING_INDICATOR)).should('not.exist');
          cy.get(CyUtils.getSelector(CypressIds.COMMON_FULL_CALENDAR_COMPONENT)).should(
            'contain',
            `${data.apply_schedule_comment_2}`
