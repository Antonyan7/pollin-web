...
Cypress.Commands.add('ChooseDateAndTime', (date, time) => {
    ...
    cy.get(CyUtils.getSelector(CypressIds.MODAL_APPOINTMENTS_ADD_DATE_AND_START_TIME))
        .should('exist').click()

    cy.get(CyUtils.getSelector(CypressIds.COMMON_TIME_PICKER_MODAL_DIALOG_COMPONENT)).should('be.visible')
    cy.get(CyUtils.getSelector(CypressIds.COMMON_TIME_PICKER_MODAL_DIALOG_COMPONENT))
        .contains(dateToSelect).click()
    cy.get(CyUtils.getSelector(CypressIds.COMMON_TIME_PICKER_MODAL_DIALOG_COMPONENT))
        .contains(time).realClick()
});
export {}