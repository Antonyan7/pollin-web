...
const Infections = () => {
  ...
  const fieldName = `${GenitourinaryFields.Infections}.value`;

  const { field } = useController({
    name: fieldName,
    control
  });
  const InfectionsLabel = t(
    Translation.PAGE_PATIENT_PROFILE_MEDICAL_BACKGROUND_MEDICAL_HISTORY_GENITOURINARY_HISTORY_FIELD_INFECTIONS
  );
  const { onChange } = field;
  const patientInformation = useAppSelector(patientsSelector.malePatientGenitourinaryHistory);
  const infections = patientInformation?.infections;

  const onConceptionChange = (hasInfections: boolean) => {
    onChange(hasInfections);
  };
  const [showAdditionalNote, setShowAdditionalNote] = useState(!!infections?.note);
  const onNoteClick = () => {
  ...
  return (
    <Grid container item px={paddings.leftRight24} py={paddings.topBottom16} direction="row" xs={12}>
      ...
      >
        <ConsultationTitleWithIcon onClick={onNoteClick} description={InfectionsLabel} />
      </Grid>
      <Grid item container xs={7}>
        ...
        <MedicalBackgroundNote
          ...
          fieldName={GenitourinaryFields.Infections}
        />
      </Grid>
    </Grid>
  );
