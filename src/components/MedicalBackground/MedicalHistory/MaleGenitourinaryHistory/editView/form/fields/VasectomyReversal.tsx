...
const VasectomyReversal = () => {
  const [t] = useTranslation();
  const { control } = useFormContext();
  const fieldName = `${GenitourinaryFields.VasectomyReversal}.value`;
  const patientInformation = useAppSelector(patientsSelector.malePatientGenitourinaryHistory);
  const vasectomyReversal = patientInformation?.vasectomyReversal;
  const { field } = useController({
    name: fieldName,
    control
  });
  const VasectomyReversalLabel = t(
    Translation.PAGE_PATIENT_PROFILE_MEDICAL_BACKGROUND_MEDICAL_HISTORY_GENITOURINARY_HISTORY_FIELD_VASECTOMY_REVERSAL
  );
  const { onChange } = field;

  const onConceptionChange = (HadVasectomyReversal: boolean) => {
    onChange(HadVasectomyReversal);
  };
  const [showAdditionalNote, setShowAdditionalNote] = useState(!!vasectomyReversal?.note);
  const onNoteClick = () => {
  ...
  return (
    <Grid container item px={paddings.leftRight24} py={paddings.topBottom16} direction="row" xs={12}>
      ...
      >
        <ConsultationTitleWithIcon onClick={onNoteClick} description={VasectomyReversalLabel} />
      </Grid>
      <Grid item container xs={7}>
        ...
        <MedicalBackgroundNote
          ...
          fieldName={GenitourinaryFields.VasectomyReversal}
        />
      </Grid>
    </Grid>
  );
