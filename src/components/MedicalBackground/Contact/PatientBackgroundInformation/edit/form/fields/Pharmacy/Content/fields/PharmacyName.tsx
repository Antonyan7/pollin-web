...
const PharmacyName = () => {
  ...
  const label = t(
    Translation.PAGE_PATIENT_PROFILE_MEDICAL_BACKGROUND_CONTACT_BACKGROUND_INFORMATION_PHARMACY_FIELDS_NAME
  );
  const { control } = useFormContext();
  const { field, fieldState } = useController({
    name: `${BackgroundInformationFormFields.Pharmacy}.pharmacyName`,
    control
  });
  const errorHelperText = generateErrorMessage(label);
  const pharmacyNameRef = useRef<HTMLInputElement>(null);

  useScrollIntoView(pharmacyNameRef, fieldState);
