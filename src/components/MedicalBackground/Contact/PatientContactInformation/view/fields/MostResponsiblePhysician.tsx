...
const MostResponsiblePhysician = ({ componentIndex }: GeneralHealthComponentsProps) => {
  const [t] = useTranslation();
  const contactInformation = useAppSelector(patientsSelector.contactInformation);
  const fieldValue = contactInformation?.responsiblePhysician;
  const fieldName = t(
...
export default MostResponsiblePhysician;
